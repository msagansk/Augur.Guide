...
Note that these examples don't include the simple example of just creating an [order]({{ "/" | absolute_url }}/glossary.html#Order) on the [order book]({{ "/" | absolute_url }}/glossary.html#Order%20Book). Remember, you need a counterparty for an actual trade to occur.

In all of the examples shown, the market in question has two [outcomes]({{ "/" | absolute_url }}/glossary.html#Outcome), A and B. You can think of these as "Yes" and "No" for a [Yes/No market]({{ "/" | absolute_url }}/glossary.html#Yes/No%20Market), or "Long" vs. "Short" for a [scalar market]({{ "/" | absolute_url }}/glossary.html#Scalar%20Market). [Categorical markets]({{ "/" | absolute_url }}/glossary.html#Categorical%20Market) will behave slightly differently, since there are more than 2 outcomes in a [complete set]({{ "/" | absolute_url }}/glossary.html#Complete%20Set). This list of examples is not exhaustive for all the possibilities in Augur, but covers the possibilities for a two outcome market. In the examples below, note that [bidding/buying]({{ "/" | absolute_url }}/glossary.html#Bid%20Order) a SHARE of A for price "x" is equivalent to [asking/selling]({{ "/" | absolute_url }}/glossary.html#Ask%20Order) a SHARE of B for price "1-x".
