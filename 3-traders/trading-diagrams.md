...
Note that these examples don't include the simple example of just creating an [order]({{ "/" | absolute_url }}/7-glossary.html#Order) on the [order book]({{ "/" | absolute_url }}/7-glossary.html#Order_Book). Remember, you need a counterparty for an actual trade to occur.

In all of the examples shown, the market in question has two [outcomes]({{ "/" | absolute_url }}/7-glossary.html#Outcome), A and B. You can think of these as "Yes" and "No" for a [Yes/No market]({{ "/" | absolute_url }}/7-glossary.html#Yes/No_Market), or "Long" vs. "Short" for a [scalar market]({{ "/" | absolute_url }}/7-glossary.html#Scalar_Market). [Categorical markets]({{ "/" | absolute_url }}/7-glossary.html#Categorical_Market) will behave slightly differently, since there are more than 2 outcomes in a [complete set]({{ "/" | absolute_url }}/7-glossary.html#Complete_Set). This list of examples is not exhaustive for all the possibilities in Augur, but covers the possibilities for a two outcome market. In the examples below, note that [bidding/buying]({{ "/" | absolute_url }}/7-glossary.html#Bid_Order) a SHARE of A for price "x" is equivalent to [asking/selling]({{ "/" | absolute_url }}/7-glossary.html#Ask_Order) a SHARE of B for price "1-x".
